...
class PlexLoginWindow {
  ...
  show() {
    ...
    this._window = window.open(
      '',
      'PlexSSO',
      `scrollbars=yes, width=${600}, height=${700}, top=${top}, left=${left}`
    );
    if (window.focus) {
      this._window.focus();
    }
...
class PlexOAuth extends EventEmitter {
  ...
  async _getPlexToken() {
    try {
      ...
      this._window.goTo(
        `https://app.plex.tv/auth/#!?clientID=${PlexHeaders['X-Plex-Client-Identifier']}&code=${code}`
      );
  ...
  async login(rememberMe, service, location, existingToken = null) {
    ...
    const csrfCookie =
      Cookie.getCookiesForString(document.cookie)
