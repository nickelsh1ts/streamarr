...
const CoreApp: Omit<NextAppComponentType, 'origGetInitialProps'> = ({
  ...
  if (router.pathname.match(/(^\/$|login|setup|resetpassword|help)/)) {
...
CoreApp.getInitialProps = async (initialProps) => {
  ...
  if (ctx.res) {
    ...
    const response = await axios.get<PublicSettingsResponse>(
      `http://localhost:${process.env.PORT || 6064}/api/v1/settings/public`
    );
    ...
    if (!initialized) {
      ...
      try {
        ...
        const response = await axios.get<User>(
          `http://localhost:${process.env.PORT || 6064}/api/v1/auth/me`,
          {
            headers:
