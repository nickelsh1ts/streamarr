    ...
    <div className={`relative ${subText ? 'h-8' : 'h-4'} w-full`}>
      ...
      >
        <input
          ...
          max={max}
          value={valueMin}
          className={`pointer-events-none absolute h-2 w-full cursor-pointer appearance-none rounded-lg bg-purple-700 ${
      ...
      <div
        className="pointer-events-none absolute top-0 z-30 ml-1 mr-1 h-2 bg-purple-500"
        style={{
          left: `${minThumb}%`,
          right: `${100 - maxThumb}%`,
        }}
      />
      {subText && (
        <div className="relative top-4 z-30 flex w-full justify-center text-sm text-purple-400">
          <span>{subText}</span>
        </div>
      )}
    </div>
