...
authRoutes.get('/me', isAuthenticated(), async (req, res) => {
  ...
  if (!req.user) {
    res.status(500).json({ status: 500, error: 'Please sign in.' });
  }
    ...
    if (!user && !(await userRepository.count())) {
      ...
      if (
        ...
        if (user) {
          ...
          logger.info(
            'Sign-in attempt from Plex user with access to the media app; creating new Streamarr user',
            {
              label: 'API',
              ip: req.ip,
              email: account.email,
        ...
        logger.warn(
          'Failed sign-in attempt by Plex user without access to the media app',
          {
            label: 'API',
            ip: req.ip,
            email: account.email,
